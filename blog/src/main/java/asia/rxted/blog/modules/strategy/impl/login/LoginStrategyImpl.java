...
public class LoginStrategyImpl extends AbstractLoginStrategyImpl {
    ...
    @Override
    public ResultMessage<UserDetailsDTO> getUserDetailsDTO(String data, HttpServletRequest request, Integer loginType) {

        LoginVO loginVO = JSON.parseObject(data, LoginVO.class);

        ResultMessage<UserInfo> result = loginHandler(loginVO, loginType);
        ...
        return ResultVO.data(userDetailsService.convertUserDetail(userAuth, request));
    }

    private ResultMessage<UserInfo> loginHandler(LoginVO loginVO, Integer loginType) {
